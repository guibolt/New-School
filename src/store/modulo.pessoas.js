...
export const moduloPessoas = {
  ...
  actions: {
    async cadastrarPessoa({commit},  pessoa){
      ...
      commit('loading')
      await apiClient.post(pessoa.tipo,pessoa).then(resp =>{
    ...
    async cadastrarTurma({commit}, turma){
      commit('loading')
      await apiClient.post('Turmas',turma).then(resp=>{
        console.log('virá ?:', resp)
  ...
  async buscarPessoas({commit}, tipo){
    commit('loading')
    await apiClient.get(tipo).then(resp =>{
  ...
  async mudarPessoa({commit}, pessoa){
    commit('loading')
    await apiClient.put(`${pessoa.tipo}/${pessoa.id}`,pessoa).then(resp =>{
  ...
  async deletarPessoa({commit}, pessoa){
    commit('loading')
    await apiClient.delete(`${pessoa.tipo}/${pessoa.id}`).then(resp =>{
  ...
  async buscarTurmas({commit}){
    commit('loading')
    await apiClient.get('Turmas').then(resp =>{
  ...
  async buscarTurmaID({commit},turma){
    commit('loading')
    await apiClient.get(`Turmas/${turma.id}`).then(resp =>{
